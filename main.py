...
class CanvasImage:
    ...
    def __init__(self, placeholder, path):
        ...
        # Bind event to show coordinates popup when mouse button is pressed
        self.canvas.bind('<Double-Button-1>', lambda event: self.show_coordinates_popup(event)) 
    ...
    def show_coordinates_popup(self, event):
        """ Show a popup window with the coordinates of the point clicked with the mouse """
        x = self.canvas.canvasx(event.x)  # get coordinates of the event on the canvas
        y = self.canvas.canvasy(event.y)
        messagebox.showinfo("Coordinates", f"X: {x}, Y: {y}")
